...
- GitOps strategy - i.e.- we abstract away k8s and make it simple to use for a Junior or mid-level developers
...
- All code is built using our CLI (`npx gsys` for no deps installs) 
- All builds are triggered by a condition, action, and event chain that is langage agnostic, stream-ready, and  business use case andration 

- Developer communication will improve, as business requirements become a graph tree of powerful functionality across the stack
- LSP provides direct integration with the IDE including auto-comletion and other AI driven code-introspection features.
